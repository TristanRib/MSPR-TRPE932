...
_MODEL_PATTERN    = re.compile(r"^model_(?!.+_(p10|p90)_).+_\d{8}_\d{6}\.pkl$")
_QUANTILE_PATTERN = re.compile(r"^model_.+_(p10|p90)_\d{8}_\d{6}\.pkl$")
...
def load_quantile_models(models_dir: str):
    ...
    p10 = sorted([f for f in files if re.match(r"^model_.+_p10_\d{8}_\d{6}\.pkl$", f)], reverse=True)
    p90 = sorted([f for f in files if re.match(r"^model_.+_p90_\d{8}_\d{6}\.pkl$", f)], reverse=True)
